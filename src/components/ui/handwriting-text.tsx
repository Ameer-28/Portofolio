...
interface OpenTypeLib {
  parse: (buffer: ArrayBuffer) => OpenTypeFont;
}

// The library, loaded once per page.
let libPromise: Promise<OpenTypeLib> | null = null;

function loadOpentype(): Promise<OpenTypeLib> {
  if (typeof window === "undefined") return Promise.reject(new Error("no window"));
  const existing = (window as unknown as { opentype?: OpenTypeLib }).opentype;
  ...
  if (!libPromise) {
    libPromise = new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.onload = () => {
        const lib = (window as unknown as { opentype?: OpenTypeLib }).opentype;
        if (lib) resolve(lib);
...
function loadFont(url: string): Promise<OpenTypeFont> {
...
export function HandwritingText({
  ...
  // Compute geometry synchronously whenever font or current word changes
